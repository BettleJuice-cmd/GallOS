...
class Articulo:
    ...
    @staticmethod
    def modificar(cod, nuevo_cod=None, nuevo_nombre=None, nueva_cant=None, nuevo_precio=None):
        if not os.path.exists("Inventario.json"):
            return False
        with open("Inventario.json", "r", encoding="utf-8") as f:
            articulos = json.load(f)
...
def guardar_venta(lista_venta, inventario, total_venta, cliente):
...
def mostrar_factura(numero_factura):
    os.system("cls")
...
def anular_factura_fichero(numero_factura):
    os.system("cls")
